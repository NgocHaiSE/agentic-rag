...
SUPPORTED_DOCUMENT_EXTENSIONS = TEXT_EXTENSIONS | ocr.SUPPORTED_EXTENSIONS
...
class DocumentIngestionPipeline:
    ...
    async def _ingest_single_document(self, file_path: str) -> IngestionResult:
        ...
        # Read document (with OCR support as needed)
        document_content, read_metadata = self._read_document(file_path)
    ...
    def _find_document_files(self) -> List[str]:
        ...
        suffixes = set(SUPPORTED_DOCUMENT_EXTENSIONS)
        suffixes.update(ext.upper() for ext in SUPPORTED_DOCUMENT_EXTENSIONS)
        patterns = sorted(f"*{ext}" for ext in suffixes)
        files = []
    ...
    def _read_document(self, file_path: str) -> Tuple[str, Dict[str, Any]]:
        """Read document content from file, using OCR when necessary."""
        path = Path(file_path)
        ...
        if suffix in TEXT_EXTENSIONS:
            try:
                with open(file_path, "r", encoding="utf-8") as f:
                    return f.read(), {"used_ocr": False}
            except UnicodeDecodeError:
                with open(file_path, "r", encoding="latin-1") as f:
                    return f.read(), {"used_ocr": False}

        if ocr.is_supported(path):
            result = ocr.extract_text(path)
            text = result.text.strip()
            if not text:
                raise ocr.OCRError(f"No text extracted from OCR for file: {file_path}")
            extra = {
                "used_ocr": result.metadata.get("used_ocr", True),
                "ocr": result.metadata,
                "ocr_languages": result.metadata.get("languages"),
                "ocr_engine": result.metadata.get("engine"),
            }
            return text, extra

        raise ocr.OCRError(f"Unsupported file type for ingestion: {file_path}")
    ...
    def _extract_document_metadata(
        ...
        if extra_metadata:
            ocr_metadata = extra_metadata.get("ocr") if isinstance(extra_metadata, dict) else None
            metadata.update({k: v for k, v in extra_metadata.items() if k != "ocr"})
            if ocr_metadata:
    ...
    async def _save_to_postgres(
        ...
        async with db_pool.acquire() as conn:
            async with conn.transaction():
                ...
                author_id = extra.get("author_id")
                effective_date = extra.get("effective_date")  # Expect ISO date string or None
                # Normalize effective_date to Python date if string provided
    ...
    try:
        ...
        if extra_fields:
            ...
            if extra_fields.get("version"):
                metadata["version"] = extra_fields["version"]
